...
function Kitty:check_kitty()
  if not vim.env.KITTY_LISTEN_ON or #vim.env.KITTY_LISTEN_ON == 0 then
    error(
      "Kitty provider selected but KITTY_LISTEN_ON environment variable is not set. Enable remote control in kitty.",
      0
    )
  end
end
